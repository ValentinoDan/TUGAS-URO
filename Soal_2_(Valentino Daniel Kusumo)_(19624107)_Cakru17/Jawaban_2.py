    ...
    def start_game(self):
        ...
        while True:
            try: 
                ...
                if 0 < first_choice < len(self.robots):
                    robot1 = self.robots[first_choice]
                    break
                else :
                    print(f"Tolong masukkan angka mulai dari 1 hingga {len(self.robots)}")
            except ValueError:
                print("Tolong masukkan sebuah angka")
        # Pemilihan robot kedua
        while True:
            try:
                second_choice = int(input("Select the second robot: ")) - 1
                if 0 < second_choice < len(self.robots):
